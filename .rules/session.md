...
모든 Exercise는 Concept Gate를 통과한 뒤 아래 형식으로 제시한다. 코드나 글 중 한 종류만 필요한 Exercise도 해당 사실을 명시하며, 두 종류를 한 목록에 섞지 않는다.
...
1. `코드 과제`라는 제목
2. 코드에서 수행할 작업을 `C1`, `C2`, `C3`처럼 식별 가능한 문항으로 구분
3. 사용할 데이터 파일, 필수 컬럼, 날짜 범위, feature/target과 split 경계
4. 반드시 저장하거나 출력해야 하는 중간 점검값과 최종 산출물
5. 코드 제출 경로
6. 코드 최소 통과 기준

코드 최소 통과 기준에는 실행 가능 여부, 핵심 객체나 컬럼, 시간 정렬, leakage 점검, 공통 평가 행과 필요한 출력이 무엇인지 적는다. 학습자가 아직 구현해야 하는 완성 코드는 제공하지 않는다.

코드 제출이 필요하지 않으면 다음처럼 명시한다.

`코드 과제: 없음 — 이번 Exercise는 개념 설명만 평가한다.`
...
1. `글 과제`라는 제목
2. 답해야 할 질문을 `T1`, `T2`, `T3`처럼 하나씩 분리
3. 각 질문에 사용할 실제 컬럼명, 예시 날짜, prediction time, target time 등 필요한 구체 조건
4. 각 질문의 기대 답변 범위
5. 표현 수준이 아니라 핵심 개념을 판별할 수 있는 질문별 최소 통과 기준
6. 글 제출 경로

코드 결과를 글로 해석해야 한다면 사용할 지표명, 비교 대상과 답해야 할 판단을 질문에 직접 적는다. 일반적인 `결과를 해석하라`만 제시하고 리뷰에서 baseline 선택 이유, 일반화 한계, 인과관계 또는 운영 제약을 새 통과 조건으로 추가하지 않는다.

글 제출이 필요하지 않으면 다음처럼 명시한다.

`글 과제: 없음 — 이번 Exercise는 실행 결과만 평가한다.`

### 3. 성찰 질문

- 성찰 질문이 글 답안의 필수 제출물이라면 `T` 문항으로 포함하고 최소 통과 기준을 적는다.
- 대화용 선택 질문이면 `제출 불필요`라고 명시한다.
- 제출 여부를 적지 않은 성찰 질문은 통과 조건으로 사용할 수 없다.

### 4. 제출 전 자체 점검

Exercise를 보내기 전에 다음을 확인한다.

- 코드에서 요구한 작업이 모두 `C` 문항에 있는가?
- 글에서 채점할 내용이 모두 `T` 문항에 질문으로 적혀 있는가?
- 각 문항의 최소 통과 기준이 사전에 공개됐는가?
- 코드 파일과 글 파일의 경로가 서로 구분됐는가?
- 새 API나 개념이 Concept Gate에서 먼저 설명됐는가?

하나라도 충족되지 않으면 Exercise를 제시하지 않고 형식을 먼저 보완한다.
...
- 실습 요구사항과 완료 기준을 구분해 설명하고, 코드 요구사항과 글 요구사항을 별도 섹션으로 제시한다.
...
- 글 답안 문항에는 질문별 최소 통과 기준을 명시한다. 예: "2023-05를 feature month로, 2023-06을 target month로 직접 언급할 것."
- 리뷰에서 평가할 내용은 반드시 사전에 `C` 또는 `T` 문항과 최소 통과 기준으로 공개한다.
