...
3. 새 항목이 하나라도 있으면 Exercise를 제시하기 전에 각 항목을 다음 순서로 설명한다.
   - 왜 지금 이 개념이 필요한지
   - 기존 표현이나 방법은 무엇을 놓치거나 잘못 표현하는지
   - 적용 전후에 데이터 표현 또는 모델이 받는 정보가 어떻게 달라지는지
   - 어떤 데이터·조건에서 유용하고 어떤 경우에는 불필요하거나 해로울 수 있는지
   - 이번 Exercise에서 어떤 비교, 지표 또는 출력으로 유용성을 검증할 것인지
   - 사람이 손으로 한다면 어떤 작업인지
...
8. 설명을 마친 뒤에는 학습자가 별도 질문 없이 다음 세 문장을 말할 수 있는지 자체 점검한다.
   - 이 개념을 왜 사용하는가?
   - 적용하면 모델이 받는 정보가 무엇이 달라지는가?
   - 이번 데이터에서 도움이 되었는지를 어떻게 판단하는가?
9. 위 세 질문 중 하나라도 Concept 설명에서 답할 수 없으면 Concept Gate를 통과하지 않은 것으로 보고 Exercise를 제시하지 않는다.
