...
- 역할: CSV 파일을 Pandas DataFrame으로 불러온다.
- 왜 필요한가: 원본 경제 데이터를 Pandas에서 다룰 수 있는 표 형태로 가져오기 위해 필요하다.
- 주요 인수:
  - 첫 번째 인수 또는 `filepath_or_buffer`: 불러올 CSV 파일 경로.
- 반환값: CSV 내용을 담은 `DataFrame`.
- 원본을 바꾸는가: 외부 CSV 파일 자체를 바꾸지 않는다.
- 다시 저장해야 하는가: 반환된 DataFrame을 변수에 저장해야 이후 작업에 사용할 수 있다.

#### `pd.to_datetime()`

- 역할: 문자열 날짜를 Pandas 날짜 타입으로 변환한다.
- 왜 필요한가: 시계열 데이터에서 날짜순 정렬과 시간 기준 해석을 하기 위해 필요하다.
- 주요 인수:
  - 날짜로 변환할 Series 또는 값.
- 반환값: 날짜 타입으로 변환된 Series 또는 DatetimeIndex.
- 원본을 바꾸는가: 원본 컬럼을 자동으로 바꾸지 않는다.
...
- 역할: DataFrame을 특정 컬럼 기준으로 정렬한다.
- 왜 필요한가: `shift(-1)`이 다음 행을 가져오기 때문에, 먼저 데이터가 시간순으로 정렬되어 있어야 한다.
- 주요 인수:
  - `by`: 정렬 기준 컬럼.
  - `ascending`: 오름차순 여부. 날짜 기준 오름차순은 과거에서 최근 순서다.
- 반환값: 정렬된 새 DataFrame.
- 원본을 바꾸는가: 기본적으로 원본을 직접 바꾸지 않는다.
- 다시 저장해야 하는가: 정렬 결과를 계속 쓰려면 변수에 다시 저장해야 한다.
