...
### Week 2 Exercise 5 - Baseline Evaluation With MAE/MSE/RMSE
...
- `answers/code/week2/week2_5.ipynb`
- `answers/text/week2/week2_5.txt`

실습 목표:

- 기준 모델의 예측값 `baseline_pred_next_month`와 실제 정답 `target_next_month`의 차이를 계산한다.
- test 구간을 feature month 기준 `2024-01-31`부터 `2024-11-30`까지 11행으로 제한한다.
- `2024-12-31`은 다음 달 정답이 없으므로 평가에서 제외해야 함을 설명한다.
- MAE, MSE, RMSE의 계산 의미를 구분한다.
- RMSE가 큰 오차를 더 민감하게 반영하면서 원래 target 단위로 돌아온다는 점을 해석한다.

배운 내용:

- 오차(error)는 이번 실습에서 `target_next_month - baseline_pred_next_month`로 계산했다.
- 절대 오차(absolute error)는 오차의 부호를 제거한 값이다.
- 제곱 오차(squared error)는 오차를 제곱한 값이다.
- MAE는 absolute error의 평균이다.
  - 이번 기준 모델의 MAE는 약 `0.4455`였다.
- MSE는 squared error의 평균이다.
  - 이번 기준 모델의 MSE는 약 `0.2118`이었다.
  - MSE는 단위가 제곱되므로 원래 산업생산지수 포인트 단위로 바로 해석하기 어렵다.
- RMSE는 MSE에 제곱근을 씌운 값이다.
  - 이번 기준 모델의 RMSE는 약 `0.4602`였다.
  - RMSE는 큰 오차를 더 민감하게 반영하면서도 원래 target과 같은 단위로 해석할 수 있다.
- 예를 들어 `2024-01-31` feature month에서는:
  - prediction time: `2024-01-31`
  - target time: `2024-02-29`
  - 기준 모델 예측값: `108.4`
  - 실제 정답: `108.9`
  - absolute error: `0.5`
- `target_next_month`를 예측값으로 복사하면 모든 오차가 0이 될 수 있지만, 이는 정답을 미리 본 것이므로 Data Leakage다.
...
- `.sort_values()`: `shift(-1)` 전에 날짜순 정렬을 보장하는 데 사용했다.
- `.drop_duplicates()`: 중복 행이 다음 target 정렬을 왜곡하지 않도록 사용했다.
- `.shift(-1)`: 다음 행의 `industrial_production_index`를 현재 행의 `target_next_month`로 가져오는 데 사용했다.
- `.dropna(subset=["target_next_month"])`: 다음 달 정답이 없는 행을 평가 후보에서 제외하는 데 사용했다.
- Boolean Filtering과 `&`: train/test 날짜 구간을 나누는 데 사용했다.
- `.copy()`: test 데이터를 독립적인 DataFrame으로 다루기 위해 사용했다.
...
- 역할: 숫자의 절대값을 반환한다.
- 왜 필요한가: 예측 오차에는 양수와 음수가 섞이므로, 평균을 내기 전에 오차 크기만 보려면 부호를 제거해야 한다.
- 주요 인수: 이번 실습에서는 인수 없이 사용했다.
- 반환값: 절대값으로 바뀐 새 Series.
- 원본을 바꾸는가: 원본 Series를 직접 바꾸지 않는다.
- 다시 저장해야 하는가: 결과를 계속 쓰려면 `test["absolute_error"] = ...`처럼 새 컬럼에 저장해야 한다.
- 모델링 주의점: absolute error는 오차의 방향을 잃는다. 과대 예측인지 과소 예측인지도 보고 싶다면 원래 `error` 컬럼을 함께 확인해야 한다.

#### `.mean()`

- 역할: Series 값의 평균을 계산한다.
- 왜 필요한가: 여러 test 행의 오차를 하나의 평가 숫자로 요약하기 위해 필요하다.
- 주요 인수: 이번 실습에서는 인수 없이 사용했다.
- 반환값: 평균 숫자 하나.
- 원본을 바꾸는가: 원본 Series나 DataFrame을 직접 바꾸지 않는다.
- 다시 저장해야 하는가: 결과를 계속 쓰려면 `MAE`, `MSE` 같은 변수에 저장해야 한다.
- 모델링 주의점: 평균은 평가 대상 행이 정확히 잡혀 있을 때만 의미가 있다. 정답이 없는 행이나 잘못된 test 구간이 섞이면 평가 지표가 왜곡된다.

#### `** 2`

- 역할: 각 오차 값을 제곱한다.
- 왜 필요한가: MSE와 RMSE는 큰 오차를 더 강하게 반영하기 위해 제곱 오차를 사용한다.
- 반환값: 제곱된 값들의 새 Series.
- 원본을 바꾸는가: 원본 Series를 직접 바꾸지 않는다.
- 다시 저장해야 하는가: 결과를 계속 쓰려면 `test["squared_error"] = ...`처럼 저장해야 한다.
- 모델링 주의점: 큰 오차가 지표에 더 크게 반영되므로, RMSE가 MAE보다 많이 크면 특정 시점의 큰 예측 실패를 확인해야 한다.

#### `np.sqrt()`

- 역할: 제곱근을 계산한다.
- 왜 필요한가: MSE에 제곱근을 씌워 RMSE를 만들고, 지표 단위를 원래 target 단위로 되돌리기 위해 사용한다.
- 주요 인수: 제곱근을 구할 숫자 또는 배열 형태의 값.
- 반환값: 제곱근 결과.
- 원본을 바꾸는가: 입력값을 직접 바꾸지 않는다.
- 다시 저장해야 하는가: 결과를 계속 쓰려면 `RMSE = ...`처럼 변수에 저장해야 한다.
- 모델링 주의점: RMSE가 낮더라도 Data Leakage가 있으면 의미 없는 성능이다. 특히 target을 예측값으로 복사했는지 확인해야 한다.

수정된 실수:

- 처음에는 `squared_error`의 평균을 `MAE` 변수에 저장했지만, 이후 MSE와 MAE를 구분해 수정했다.
- 처음에는 기준 모델이 target month 값을 가져온다는 식으로 표현했지만, 이후 feature month의 현재 `industrial_production_index`를 예측값으로 사용한다고 교정했다.
- 처음에는 text 답안에 지표 숫자를 직접 쓰지 않았지만, 이후 코드 출력의 MAE, MSE, RMSE 값을 글 답안에 옮겼다.
- MSE는 단위가 제곱되어 직접 해석하기 어렵고, RMSE는 제곱근을 통해 원래 target 단위로 돌아온다는 점을 정리했다.

핵심 정리:

- 기준 모델 평가는 예측값과 실제 정답의 차이를 test 구간에서 계산하는 과정이다.
- MAE는 평균적인 절대 오차 크기를 보여준다.
- MSE는 제곱 오차의 평균이며 큰 오차를 더 강하게 반영하지만 단위 해석이 어렵다.
